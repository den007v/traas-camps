...
export async function POST(req: Request) {
  ...
  if (scores.length !== itLandscapeQuestions.length) {
  ...
  const { error } = await supabase.from("assessment_it_landscape_sessions").insert({
    role: body.role ?? null,
  ...
  if (error) {
    console.error("[assessment-it-landscape]", error.message);
