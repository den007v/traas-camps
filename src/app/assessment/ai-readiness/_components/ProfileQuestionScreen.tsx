...
import type { ProfileQuestion } from "../_lib/types";

const optionLetters = ["A", "B", "C", "D", "E", "F"];

export function ProfileQuestionScreen({
  question,
  questionIndex,
  totalSteps,
  selectedValue,
      ...
      <Container className="flex min-h-[calc(100vh-4rem)] max-w-3xl flex-col">
        <div className="space-y-4">
          <div className="flex items-center justify-between gap-3 text-sm text-[var(--muted)]">
            <Link href="/" className={`${assessmentChip} min-h-9 gap-1.5`}>
              <Home className="h-3.5 w-3.5" aria-hidden="true" />
              TraaS
            </Link>
            <span aria-live="polite">Шаг {current} из {totalSteps}</span>
        ...
        <div className="sticky bottom-0 -mx-4 flex border-t border-[var(--divider)] bg-[color:color-mix(in_oklab,var(--background)_88%,transparent)] px-4 py-4 sm:static sm:mx-0 sm:border-t-0 sm:bg-transparent sm:px-0">
          <button type="button" onClick={onBack} className={`${assessmentSecondaryButton} w-full sm:w-auto`}>
