  ...
  return (
    <footer id="footer" className="border-t border-[var(--divider)] bg-[var(--surface-soft)]">
      <Container className="py-14">
              ...
              <ul className="mt-4 space-y-2">
                {col.links.map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="text-sm text-[var(--muted)] transition hover:text-[#ff5a65]"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <p className="mt-12 border-t border-[var(--divider)] pt-8 text-xs leading-relaxed text-[var(--muted)]">
