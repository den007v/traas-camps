...
export function useNeuralNetCanvas(canvasRef: RefObject<HTMLCanvasElement | null>) {
  ...
  const resetScene = useCallback((width: number, height: number) => {
    ...
    for (let i = 0; i < 38; i += 1) {
      ...
      nodes.push({
        x: i === 0 ? width / 2 : random(inset, width - inset),
        y: i === 0 ? height / 2 : random(inset, height - inset),
  ...
  useEffect(() => {
    const draw = () => {
      ...
      if (t % 40 === 0 && edges.length && pulses.length < 12) {
        const [from, to] = edges[Math.floor(Math.random() * edges.length)];
        pulses.push({ from, to, progress: 0, speed: random(0.008, 0.018) });
      }
      ...
      for (let i = 1; i < nodes.length; i += 1) {
        ...
        n.x += n.vx;
        n.y += n.vy;
        if (n.x <= inset + n.r || n.x >= width - inset - n.r) n.vx *= -1;
        if (n.y <= inset + n.r || n.y >= height - inset - n.r) n.vy *= -1;
